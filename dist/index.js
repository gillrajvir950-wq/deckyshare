    ...
    return function Panel() {
        ...
        async function startShortcutPairing() {
            ...
            try {
                ...
                if (!r || !r.ok)
                    throw new Error(r && r.error || "Could not start pairing");
                setShortcutPair(r);
                setErr("");
            }
            catch (e) {
                setErr("Share Sheet pairing: " + String(e && e.message || e));
            }
            finally {
                setShortcutPairBusy(false);
            }
        }
        useEffect(() => { bootstrap(); }, []);
        useEffect(() => { if (!status)
