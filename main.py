...
</style></head><body><div class="wrap"><h1>DeckyShare <span style="font-size:12px;opacity:.65">v1.0.2-dev</span></h1><div class="muted">Share files with your Steam Deck. Simple. Wireless. Fast.</div>
...
async function uploadOne(f,onProgress){{let chunk=4*1024*1024,off=0,lastName=f.name;while(off<f.size){{let end=Math.min(f.size,off+chunk);let blob=f.slice(off,end);let r=await api('/upload?name='+encodeURIComponent(f.name)+'&offset='+off+'&total='+f.size,{{method:'PUT',headers:{{'Content-Type':'application/octet-stream'}},body:blob}});let j=await r.json();off=j.received;lastName=j.name||lastName;onProgress(off);}}return lastName;}}
document.getElementById('upload').onclick=async()=>{{let input=document.getElementById('file'),files=Array.from(input.files||[]),u=document.getElementById('uptext'),b=document.getElementById('upbar'),btn=document.getElementById('upload');if(!files.length)return;let total=files.reduce((n,f)=>n+f.size,0),finished=0,saved=[];btn.disabled=true;try{{for(let i=0;i<files.length;i++){{let f=files[i],name=await uploadOne(f,done=>{{let overall=total?((finished+done)*100/total):100;u.textContent=`${{i+1}}/${{files.length}} • ${{f.name}} • ${{overall.toFixed(1)}}% overall`;b.style.width=Math.min(100,overall)+'%';}});finished+=f.size;saved.push(name);}}u.textContent=`Completed ${{files.length}} file${{files.length===1?'':'s'}}`;b.style.width='100%';setTimeout(()=>{{u.textContent='';b.style.width='0%';input.value='';}},1800);}}catch(e){{u.textContent='Upload failed: '+e.message;}}finally{{btn.disabled=false;}}}};
