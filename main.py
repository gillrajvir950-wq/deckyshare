...
document.getElementById('upload').onclick=async()=>{{let f=document.getElementById('file').files[0];if(!f)return;let u=document.getElementById('uptext'),b=document.getElementById('upbar'),btn=document.getElementById('upload');btn.disabled=true;u.textContent='Uploading…';b.style.width='2%';try{{let r=await api('/upload?name='+encodeURIComponent(f.name)+'&offset=0&total='+f.size,{{method:'PUT',headers:{{'Content-Type':'application/octet-stream'}},body:f}});let j=await r.json();if(!j.complete)throw new Error('Upload incomplete');u.textContent='Completed';b.style.width='100%';setTimeout(()=>{{u.textContent='';b.style.width='0%';document.getElementById('file').value='';}},1500)}}catch(e){{u.textContent='Upload failed — '+(e.message||e);b.style.width='0%'}}finally{{btn.disabled=false}}}};
