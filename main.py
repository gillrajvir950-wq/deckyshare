...
</style></head><body><div class="wrap"><h1>DeckyShare <span style="font-size:12px;opacity:.65">v1.0.3-dev</span></h1><div class="muted">Share files with your Steam Deck. Simple. Wireless. Fast.</div>
...
<div class="card"><h2>Send to Deck</h2><p class="muted">Select one or more files. They will be sent to Downloads/DeckShare.</p><input id="file" type="file" multiple><div class="row"><button id="upload">Send selected files</button></div><div id="uptext" class="muted"></div><div class="bar"><div id="upbar" class="fill"></div></div></div>
...
async function uploadOne(f,onProgress,onRetry){{let chunk=4*1024*1024,off=0,lastName=f.name,retries=0;while(off<f.size){{let end=Math.min(f.size,off+chunk),blob=f.slice(off,end);try{{let r=await fetch('/upload?name='+encodeURIComponent(f.name)+'&offset='+off+'&total='+f.size,{{method:'PUT',headers:{{'Content-Type':'application/octet-stream'}},body:blob}});if(r.status===409){{let j=await r.json(),serverOff=Number(j.received||0);if(serverOff<0||serverOff>f.size)throw new Error('Invalid resume offset');off=serverOff;retries=0;onProgress(off);onRetry('Resuming from '+fmt(off));continue;}}if(!r.ok){{let msg=await r.text();if(r.status>=500)throw new Error(msg||('Server error '+r.status));throw new Error(msg||('Upload error '+r.status));}}let j=await r.json();off=Number(j.received||0);lastName=j.name||lastName;retries=0;onProgress(off);}}catch(e){{retries++;if(retries>3)throw e;onRetry('Connection interrupted — retry '+retries+'/3');await new Promise(resolve=>setTimeout(resolve,750*retries));}}}}return lastName;}}
document.getElementById('upload').onclick=async()=>{{let input=document.getElementById('file'),files=Array.from(input.files||[]),u=document.getElementById('uptext'),b=document.getElementById('upbar'),btn=document.getElementById('upload');if(!files.length)return;let total=files.reduce((n,f)=>n+f.size,0),finished=0;btn.disabled=true;try{{for(let i=0;i<files.length;i++){{let f=files[i],label=()=>`${{i+1}}/${{files.length}} • ${{f.name}}`;await uploadOne(f,done=>{{let overall=total?((finished+done)*100/total):100;u.textContent=label()+' • '+overall.toFixed(1)+'% overall';b.style.width=Math.min(100,overall)+'%';}},status=>{{u.textContent=label()+' • '+status;}});finished+=f.size;}}u.textContent=`Completed ${{files.length}} file${{files.length===1?'':'s'}}`;b.style.width='100%';setTimeout(()=>{{u.textContent='';b.style.width='0%';input.value='';}},1800);}}catch(e){{u.textContent='Upload failed after retries: '+e.message;}}finally{{btn.disabled=false;}}}};
...
class Handler(BaseHTTPRequestHandler):
    ...
    server_version = "DeckyShare/1.0.3-dev"
